...
import {
  ...
  SkipBack, SkipForward, QrCode, ExternalLink, Users, Clock, ShieldCheck
} from 'lucide-react';
...
export const TVDisplayView: React.FC = () => {
  ...
  // Format URL YouTube untuk tampilan TV pasif: autoplay, tanpa kontrol, caption, keyboard, fullscreen, dan rekomendasi.
  ...
  // Handler Tombol Prev & Next Video Playlist
  ...
  // Dynamic Clock Update (Asia/Makassar WITA)
  useEffect(() => {
  ...
  const handleVideoError = () => {
    console.warn("Video utama gagal dimuat, menggunakan fallback video player.");
    setVideoError(true);
  };
  ...
  return (
    <div className="tv-screen" onClick={(e) => handleUnlockAudio(e)}>
      {/* Header Bar TV Presisi (Tinggi Logo & Judul Sama Persis 68px) */}
      <header className="tv-header">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          {!isAudioUnlocked && (
            <button
              onClick={(e) => { e.stopPropagation(); handleUnlockAudio(e); }}
      ...
      {/* Main Grid TV Layout Presisi (Panel Kiri-Tengah 65.5% + Panel Panggilan Kanan 33.5%) */}
      <div className="tv-body">
        {/* PANEL KIRI + TENGAH GABUNGAN (65.5% Lebar Layar) */}
        <div className="tv-main-left-section">
          {/* BARIS ATAS: Pemutar Video 16:9 Proporsional & Kartu QR Code SKM */}
          <div className="tv-top-row-video-qr">
            {/* 1. Pemutar Video 16:9 Proporsional */}
            <div className="tv-video-container" onClick={(e) => e.stopPropagation()}>
              ...
              {/* Controls Overlay Bar Transparan */}
            ...
            <div className="tv-skm-card">
              ...
              <div className="tv-skm-qr-box">
                <img 
                  src={qrCodeApiUrl} 
                  alt="Barcode QR Code Survei Kepuasan Masyarakat 2026 DPPKB Majene" 
                  className="tv-skm-qr-img" 
                />
              </div>

              {/* Tautan Link Resmi Di Paling Bawah */}
              <div className="tv-skm-link-box">
                <ExternalLink size={12} style={{ flexShrink: 0 }} />
                <span className="tv-skm-link-text">skm.go.id/share/instansi/948f34b1...</span>
              </div>
            </div>
          </div>

          {/* BARIS BAWAH: Kartu Status Pelayanan KB & Pelayanan Sekretariat */}
          <div className="tv-bottom-cards-row">
            {/* Card Pelayanan KB (Kode A) */}
            <div className="tv-service-status-card">
              {/* Header Kartu Presisi */}
              <div className="tv-card-header-line">
                <div className="tv-card-header-left">
                  <div className="tv-icon-box-kb">
              ...
              <div className="tv-card-body-rich-stack">
                {/* Box 1: Antrean Menunggu */}
                <div className="tv-rich-stat-box-kb">
                  <div className="tv-rich-stat-label">
                    <Users size={16} /> ANTREAN MENUNGGU SAAT INI
                  </div>
                  <div className="tv-rich-stat-value-kb">
                    {String(waitingKB).padStart(2, '0')} <span className="tv-rich-stat-unit">ORANG</span>
                  </div>
                </div>

                {/* Box 2: Estimasi Waktu */}
                <div className="tv-rich-stat-box-est">
                  <div className="tv-rich-stat-label">
                    <Clock size={16} /> RATA-RATA ESTIMASI WAKTU TUNGGU
                  </div>
                  <div className="tv-rich-stat-value-est">
                    {waitingKB > 0 ? `${waitingKB * 10} MENIT` : '0 MENIT (BEBAS ANTREAN)'}
                  </div>
                </div>
              </div>

              {/* Footer Kartu Informatif & Edukatif */}
            ...
            {/* Card Pelayanan Sekretariat (Kode B) */}
            <div className="tv-service-status-card">
              {/* Header Kartu Presisi */}
              <div className="tv-card-header-line">
                <div className="tv-card-header-left">
                  <div className="tv-icon-box-sk">
                    <FileText size={26} />
                  </div>
                  <div>
              ...
              <div className="tv-card-body-rich-stack">
                ...
                <div className="tv-rich-stat-box-sk">
                  ...
                  <div className="tv-rich-stat-value-sk">
                    {String(waitingSK).padStart(2, '0')} <span className="tv-rich-stat-unit">ORANG</span>
                  </div>
                </div>
                ...
                <div className="tv-rich-stat-box-est">
                  ...
                  <div className="tv-rich-stat-value-est">
                    {waitingSK > 0 ? `${waitingSK * 5} MENIT` : '0 MENIT (BEBAS ANTREAN)'}
                  </div>
                </div>
              </div>
        ...
        {/* PANEL KANAN: Pemanggilan Aktif (Nama Loket 100% Utuh Bebas Terpotong!) */}
        <div className="tv-right-col">
          {/* Card Sedang Dipanggil Murni Realtime */}
          <div className="tv-current-call-card">
            ...
            {/* Nama Loket Utama (100% UTUH BEBAS TERPOTONG!) */}
            ...
            {latestCall ? (
              <div className="tv-call-chime-info">
                <Speaker size={16} /> Panggilan ke-{latestCall.callCount || 1}
              </div>
            ) : (
          ...
          {/* Card Antrean Berikutnya Murni Realtime */}
      ...
      {/* Footer Banner Bar dengan Running Text Marquee */}
      <footer className="tv-footer-bar">
        <div className="tv-footer-left-marquee">
          <div className="tv-marquee-badge-box">
            <div className="tv-info-icon-badge">i</div>
          </div>

          <div className="tv-marquee-container">
            <div className="tv-marquee-track">
              {[...marqueeItems, ...marqueeItems].map((item, idx) => (
                <span key={idx} className="tv-marquee-item">
                  <span className="tv-marquee-star">*</span> {item}
                </span>
              ))}
            </div>
          </div>
        ...
        <div className="tv-footer-right">
          Melayani dengan Ramah, Cepat, dan Transparan
        </div>
      </footer>
      ...
      {callOverlay?.active && (
        <div className="tv-call-overlay">
          <div className="tv-call-overlay-box">
            ...
            <p style={{ color: '#cbd5e1', marginTop: '1.2rem', fontSize: '1.4rem' }}>
              {callOverlay.serviceTitle}
            </p>
          </div>
        </div>
      )}
